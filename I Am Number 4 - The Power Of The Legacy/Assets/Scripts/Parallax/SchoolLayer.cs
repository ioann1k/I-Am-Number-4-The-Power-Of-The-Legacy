...
public class SchoolLayer
{
    ...
    public GameObject CheckPosition(Camera camera)
    {
        ...
        if (spriteRenderers.Count > 0)
        {
            ...
            if (parallaxManager.CanSpawnAnotherLayer())
            {
                Debug.Log(true);
                foreach (Renderer spriteRenderer in spriteRenderers.ToArray())
                {
                    ...
                    {
                        parallaxManager.SpawnGroup(widthOfSprite);
                    }
                    ...
                    if (spriteRenderer.transform.position.x + (spriteRenderer.bounds.size.x / 2) <= -(widthOfSprite / 2))
                    {
                        ...
                        parallaxManager.SpawnGroup(newPosition);
                    }
                }
            }
        }
    ...
    public void Update(SchoolParallaxManager parallaxManager, Camera camera, float time)
    {
        foreach (Renderer spriteRenderer in spriteRenderers.ToArray())
        {
